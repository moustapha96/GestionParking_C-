    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < place.Rows.Count; i++)
            {
                ...
                {
                    MessageBox.Show("place non disponible pour cette  voiture");
                }
            }
            actualise_combo(); actualise_combo_vp();
        }
        ...
        public void actualise_combo()
        {
            ...
            MySqlConnection connect = new MySqlConnection("SERVER=localhost; user id=root; password=; database=parking_db");
            MySqlCommand cmd = new MySqlCommand("SELECT * from voiture where etat = false ");
            cmd.CommandType = CommandType.Text;
            cmd.Connection = connect;
            connect.Open();
            try
        ...
        public void actualise_combo_vp()
        {
            ...
            DataTable dts = new DataTable();
            MySqlConnection connect = new MySqlConnection("SERVER=localhost; user id=root; password=; database=parking_db");
            string requette = "SELECT space.id as N° , voiture.matricule ,voiture.id as N°V , place.id as N°Place, " +
                              "space.temps  ,place.longueur , place.largeur " +
                                "FROM  voiture " +
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    comboBox_liste_vp.Items.Add(reader.GetString(0)+" "+
                        reader.GetInt32(2)+" "+reader.GetInt32(3)+" Matricule: "+ reader.GetString(1));
                }
            }
            catch (Exception ex)
            {
        ...
        private void button_reini_Click(object sender, EventArgs e)
        {
            Voiture v = new Voiture(); v.deleteAll();
            Place p = new Place(); p.deleteAll();
            Space s = new Space(); s.deleteAll();
        }
    }
}
